...
pub const Table = struct {
    ...
    /// Parses a whole row field-by-field, one type per field, and
    /// returns a tuple: `rowAs(&.{ []const u8, f32 }, i)` -> `.{ did,
    /// rate }`. Fields you don't care about still need a type — use
    /// `[]const u8`, it's free (verbatim slice). Allocates nothing.
    pub fn rowAs(self: Table, comptime Ts: []const type, row: usize) error{ RowOutOfBounds, FieldCountMismatch, InvalidCharacter, Overflow }!std.meta.Tuple(Ts) {
        if (row >= self.rows.len) return error.RowOutOfBounds;
        const fields = self.rows[row];
        if (fields.len != Ts.len) return error.FieldCountMismatch;
        var out: std.meta.Tuple(Ts) = undefined;
        inline for (Ts, 0..) |T, i| out[i] = try parseAs(T, fields[i]);
        return out;
    }

    /// Parses a whole row as a single type into a fresh slice. For a
    /// sub-slice (e.g. skipping an id column) use `fieldsAs` directly.
    pub fn sliceRowAs(self: Table, comptime T: type, row: usize, arena: Allocator) error{ RowOutOfBounds, OutOfMemory, InvalidCharacter, Overflow }![]T {
        if (row >= self.rows.len) return error.RowOutOfBounds;
        return fieldsAs(T, self.rows[row], arena);
    }

    /// Parses one column down all rows into a fresh slice.
    pub fn colAs(self: Table, comptime T: type, col: usize, arena: Allocator) error{ ColumnOutOfBounds, OutOfMemory, InvalidCharacter, Overflow }![]T {
        const out = try arena.alloc(T, self.rows.len);
        errdefer arena.free(out);
        for (self.rows, 0..) |r, i| {
            if (col >= r.len) return error.ColumnOutOfBounds;
            out[i] = try parseAs(T, r[col]);
        }
        return out;
    }
...
/// Parses a field slice as `[]T` — like `Table.sliceRowAs` but for a
/// sub-slice, e.g. skipping a leading id column: `fieldsAs(f32,
/// t.rows[i][1..], arena)`.
pub fn fieldsAs(comptime T: type, fields: []const []const u8, arena: Allocator) error{ OutOfMemory, InvalidCharacter, Overflow }![]T {
...
test "rowAs parses a heterogeneous row into a tuple" {
    const t = try parseTest("did\txi\tsigma\ndid:plc:abc\t0.5\t1.25\n", .tsv);
    defer t.deinit(std.testing.allocator);

    const r = try t.rowAs(&.{ []const u8, f32, f64 }, 0);
    try std.testing.expectEqualStrings("did:plc:abc", r[0]);
    try std.testing.expectEqual(@as(f32, 0.5), r[1]);
    try std.testing.expectEqual(@as(f64, 1.25), r[2]);

    try std.testing.expectError(error.RowOutOfBounds, t.rowAs(&.{[]const u8}, 9));
    try std.testing.expectError(error.FieldCountMismatch, t.rowAs(&.{ []const u8, f32 }, 0));
    try std.testing.expectError(error.InvalidCharacter, t.rowAs(&.{ f32, f32, f32 }, 0)); // did is not a number
}

test "colAs parses one column down all rows" {
    const t = try parseTest("did\trate\na\t1.5\nb\t2.5\n", .tsv);
    defer t.deinit(std.testing.allocator);

    const rates = try t.colAs(f32, 1, std.testing.allocator);
    defer std.testing.allocator.free(rates);
    try std.testing.expectEqualSlices(f32, &.{ 1.5, 2.5 }, rates);

    const dids = try t.colAs([]const u8, 0, std.testing.allocator);
    defer std.testing.allocator.free(dids);
    try std.testing.expectEqualStrings("b", dids[1]);

    try std.testing.expectError(error.ColumnOutOfBounds, t.colAs(f32, 2, std.testing.allocator));
}
...
test "getAs and rowAs error on out-of-bounds instead of panicking" {
    ...
    try std.testing.expectError(error.RowOutOfBounds, t.sliceRowAs([]const u8, 9, std.testing.allocator));
}
...
test "sliceRowAs frees its buffer when a field fails to parse" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const t = try parse(arena.allocator(), "a\tb\n1\tx\n", .tsv);

    var gpa = std.heap.DebugAllocator(.{}){};
    const out = t.sliceRowAs(u32, 0, gpa.allocator());
...
test "headerless one-row float file parses into a flat array" {
    ...
    const gaps = try t.sliceRowAs(f32, 0, a);
